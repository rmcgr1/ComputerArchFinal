...
def clean_up():
    ...
    print "Number of data cache hit " + str(stats['DC_HITS'])

    output = ''
    for r in result_list:
        if r.startswith('HLT'):
            output = output + 'HLT\t' + r[3:] + '\n'
        else:
            output = output + r + '\n'
    output = output + '\n'
    output = output + "Total number of access requests for instruction cache " + str(stats['IC_REQ']) + '\n'
    output = output + "Number of instruction cache hits " + str(stats['IC_HITS']) + '\n'
    output = output + "Total number of access requests for data cache " + str(stats['DC_REQ']) + '\n'
    output = output + "Number of data cache hit " + str(stats['DC_HITS'])
...
while True:
    ...
    if STOPPING and len(IF) == 0 and len(IF_completion) == 0 and len(ID) == 0 and len(ID_Ready) == 0 and len(EX) == 0 and len(EX_completion) == 0 and len(MEM_completion) == 0 and len(EX_Ready) == 0 and len(WB) == 0:
       clean_up()
